...
async def lifespan(app: FastAPI):
    ...
    # Load StrategyHub Address with Priority: File (Local) > Env Var > .env
    hub_address = None

    # Priority 1: Local Deployment File
    if os.path.exists("contracts/deployed_address.txt"):
        with open("contracts/deployed_address.txt", "r") as f:
            hub_address = f.read().strip()
            logger.info(f"Loaded StrategyHub from file (Local Override): {hub_address}")

    # Priority 2: Primary Environment Variable
    if not hub_address:
        hub_address = os.getenv("STRATEGY_HUB_ADDRESS")
        if hub_address:
            logger.info(f"Loaded StrategyHub from ENV: {hub_address}")
    
    # Priority 3: Fallback from .env parse (if not in process env)
            ...
            with open(".env") as f:
                for line in f:
                    if "STRATEGY_HUB_ADDRESS" in line:
                         ...
                         if len(parts) > 1:
                             hub_address = parts[1]
                             logger.info(f"Loaded StrategyHub from .env fallback: {hub_address}")
